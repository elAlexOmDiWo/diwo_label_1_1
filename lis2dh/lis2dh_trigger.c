...
static inline void setup_int1(const struct device *dev, bool enable) {
  ...
  gpio_pin_interrupt_configure_dt(
      &cfg->gpio_drdy, enable ? GPIO_INT_LEVEL_ACTIVE : GPIO_INT_DISABLE);
}
...
static int lis2dh_trigger_drdy_set(const struct device *dev,
  ...
  status =
      lis2dh->hw_tf->update_reg(dev, LIS2DH_REG_CTRL3, LIS2DH_EN_DRDY1_INT1, 0);
...
static int lis2dh_start_trigger_int1(const struct device *dev) {
  ...
  status =
      lis2dh->hw_tf->write_reg(dev, LIS2DH_REG_CTRL1, ctrl1 & ~LIS2DH_ODR_MASK);
...
static inline void setup_int2(const struct device *dev, bool enable) {
  ...
  gpio_pin_interrupt_configure_dt(&cfg->gpio_int, enable ? GPIO_INT_LEVEL_ACTIVE
                                                         : GPIO_INT_DISABLE);
}
...
static int lis2dh_trigger_anym_set(const struct device *dev,
  ...
  status = lis2dh->hw_tf->read_reg(
      dev, cfg->hw.anym_on_int1 ? LIS2DH_REG_INT1_SRC : LIS2DH_REG_INT2_SRC,
      &reg_val);
...
static int lis2dh_trigger_freefall_set(const struct device *dev, sensor_trigger_handler_t handler) {
  const struct lis2dh_config *cfg = dev->config;
  struct lis2dh_data *lis2dh = dev->data;
  int status;
  uint8_t reg_val;
  
	LOG_DBG("Set trigger FreeFall handler\n" );

  if (cfg->gpio_int.port == NULL) {
    LOG_ERR("trigger_set FreeFall int not supported");
    return -ENOTSUP;
  }
        
	setup_int1(dev, false);
//2. Запишите 00h в CTRL_REG2 // Фильтр верхних частот отключен
	status = lis2dh->hw_tf->write_reg(dev, LIS2DH_REG_CTRL2, 0x00);
	if (status < 0) {
		return status;
	}
	status = lis2dh->hw_tf->write_reg(dev, LIS2DH_REG_CTRL3, LIS2DH_EN_I1_INT1 );
	if (status < 0) {
		return status;
	}
	status = lis2dh->hw_tf->write_reg(dev, LIS2DH_REG_CTRL5, LIS2DH_EN_LIR_INT1 );
	if (status < 0) {
		return status;
	}        
        ...
        //  /* disable any movement interrupt events */
        //  status = lis2dh->hw_tf->write_reg(
        //      dev, cfg->hw.anym_on_int1 ? LIS2DH_REG_INT1_CFG :
        //      LIS2DH_REG_INT2_CFG, 0);
        //
        //  /* make sure any pending interrupt is cleared */
        //  status = lis2dh->hw_tf->read_reg(
        //      dev, cfg->hw.anym_on_int1 ? LIS2DH_REG_INT1_SRC :
        //      LIS2DH_REG_INT2_SRC, &reg_val);
	...
	lis2dh->handler_free_fall = handler;
...
static int lis2dh_start_trigger_int2(const struct device *dev) {
  ...
  return lis2dh->hw_tf->write_reg(
      dev, cfg->hw.anym_on_int1 ? LIS2DH_REG_INT1_CFG : LIS2DH_REG_INT2_CFG,
      LIS2DH_ANYM_CFG);
}
...
int lis2dh_trigger_set(const struct device *dev,
  ...
  if (trig->type == SENSOR_TRIG_DATA_READY &&
    ...
    return lis2dh_trigger_freefall_set(dev, handler);
  }
	return -ENOTSUP;
}

int lis2dh_acc_freefall_config(const struct device *dev, enum sensor_attribute attr, const struct sensor_value *val) {

// community.st.com/t5/mems-sensors/free-fall-detection-in-lis2de12-accelerometer/m-p/421370
  ...
  reg_val = 0x40;
  status = lis2dh->hw_tf->write_reg(dev, LIS2DH_REG_CTRL3, reg_val);
  if (status < 0) {
    return status;
  }

//2. Запишите 00h в CTRL_REG2 // Фильтр верхних частот отключен
  status = lis2dh->hw_tf->write_reg(dev, LIS2DH_REG_CTRL2, 0 );
  if (status < 0) {
    return status;
  }	
//3. Запишите 40h в CTRL_REG3 // Действие прерывания 1, переданное на контактную площадку INT1
//
//4. Запишите 00h в CTRL_REG4 // FS = ±2 г
//
//5. Запишите 08h в CTRL_REG5 // Прерывание 1 контакта защелкнуто
//
//6. Запишите 16 часов в INT1_THS // Установите порог свободного падения = 350 мг.
  status = lis2dh->hw_tf->write_reg(dev, LIS2DH_REG_INT1_DUR, val->val2 );
  if (status < 0) {
    return status;
  }
//7. Запишите 03h в INT1_DURATION // Установить минимальную продолжительность события
  status = lis2dh->hw_tf->write_reg(dev, LIS2DH_REG_INT1_DUR, val->val1 );
  if (status < 0) {
    return status;
  }	
//8. Записать 95h в INT1_CFG // Настроить распознавание свободного падения
//
//9. Опрос площадки INT1; если INT1 = 0, то перейти к 10 // Опрос вывода INT1 в ожидании события свободного падения
...
int lis2dh_acc_slope_config(const struct device *dev,
  ...
  if (attr == SENSOR_ATTR_SLOPE_TH) {
    ...
    LOG_INF("int2_ths=0x%x range_g=%d ums2=%u", reg_val, range_g,
            slope_th_ums2 - 1);

    status = lis2dh->hw_tf->write_reg(
        dev, cfg->hw.anym_on_int1 ? LIS2DH_REG_INT1_THS : LIS2DH_REG_INT2_THS,
        reg_val);
    ...
    status = lis2dh->hw_tf->write_reg(
        dev, cfg->hw.anym_on_int1 ? LIS2DH_REG_INT1_DUR : LIS2DH_REG_INT2_DUR,
        val->val1);
  }
...
static void lis2dh_gpio_int1_callback(const struct device *dev, struct gpio_callback *cb, uint32_t pins) {
...
static void lis2dh_thread_cb(const struct device *dev) {
  ...
  if (cfg->gpio_drdy.port && unlikely(atomic_test_and_clear_bit(
                                 &lis2dh->trig_flags, START_TRIG_INT1))) {
  ...
  if (cfg->gpio_int.port && unlikely(atomic_test_and_clear_bit(
                                &lis2dh->trig_flags, START_TRIG_INT2))) {
  ...
  if (cfg->gpio_int.port &&
      atomic_test_and_clear_bit(&lis2dh->trig_flags, TRIGGED_INT2)) {
...
int lis2dh_init_interrupt(const struct device *dev) {
  ...
  LOG_INF("%s: int1 on %s.%02u", dev->name, cfg->gpio_drdy.port->name,
          cfg->gpio_drdy.pin);
...
check_gpio_int:
  ...
  if (status < 0) {
    LOG_ERR("Could not configure %s.%02u", cfg->gpio_int.port->name,
            cfg->gpio_int.pin);
    return status;
  }

  gpio_init_callback(&lis2dh->gpio_int2_cb, lis2dh_gpio_int2_callback,
  ...
  LOG_INF("%s: int2 on %s.%02u", dev->name, cfg->gpio_int.port->name,
          cfg->gpio_int.pin);
  ...
  if (cfg->hw.anym_on_int1) {
    ...
    status = lis2dh->hw_tf->update_reg(
        dev, LIS2DH_REG_CTRL3, LIS2DH_EN_INT1_INT1, LIS2DH_EN_INT1_INT1);

    /* latch int1 line interrupt */
    status =
        lis2dh->hw_tf->write_reg(dev, LIS2DH_REG_CTRL5, LIS2DH_EN_LIR_INT1);
  } else {
    /* enable interrupt 2 on int2 line */
    status = lis2dh->hw_tf->update_reg(
        dev, LIS2DH_REG_CTRL6, LIS2DH_EN_INT2_INT2, LIS2DH_EN_INT2_INT2);

    /* latch int2 line interrupt */
    status =
        lis2dh->hw_tf->write_reg(dev, LIS2DH_REG_CTRL5, LIS2DH_EN_LIR_INT2);
  }
